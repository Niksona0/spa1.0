const translations = {
  en: {
    // Nav bar
    hometext: "Home",
    about: "About",
    service: "Service",
    masseuse: "masseuse",
    contact: "Contact",
    spa: "Spa & Beauty Center",
    h3treatment: "Magic Hands",
    treatment: "Massage treatments relax muscles, relieve tension, and improve circulation, promoting overall well-being.",
    Appointment: "Make Appointment",
    gallery: "Gallery",

    // ABOUT US
    aboutUs: "About Us",
    bestMm: "Best Masseuse",
    eur: "Experience Unmatched Relaxation and Wellness with Our Exceptional Masseuses: Elevate Your Journey to New Heights of Comfort, Renewal, and Total Relaxation",
    hsrv: "Hotels Served",
    hClients: "Happy Clients",

    // Service starts
    ourService: "Our Service",
    spaBeauty: "Spa & Beauty Services",
    
    backShoulder: "Back and Shoulder Massage",
    backShoulderMassage: "Back and shoulder massages are great for relaxing muscles and relieving tension.",
    
    headBack: "Head and Back Massage",
    headBackMassage: "Prioritize your vitality and experience a rejuvenating head and back massage today!",

    faceLifting: "Face Lifting",
    faceLiftingMassage: "Refresh and rejuvenate with a head and back massage and a face lifting treatment today!",

    lympatatic: "Lympahatic Dranage Massage",
    lympataticMassage: "Lymphatic drainage massage boosts circulation, reduces swelling, and aids detoxification.",

    deepTissue: "Deep tissue",
    deepTissueMassage: "Deep tissue massage targets deep muscles to relieve tension, ease pain, and enhance flexibility.",

    medical: "Therapeutic massage",
    medicalMassage: "Therapeutic massage is for the elimination of pain in the muscle skeletal system.",

    japanese: "Herbal Ball Massage",
    japaneseMassage: "Herbal ball massage uses heated herbal compresses to relieve muscle pain and enhance relaxation.",

    swedish: "Swedish massge",
    swedishMassage: "Swedish massage relaxes muscles with gentle pressure and long strokes, promoting well-being.",

    hotStone: "Hot stone massge",
    hotStoneMassage: "Hot stone massage uses heated stones to relax muscles and promote relaxation and well-being.",

    thai: "Thai massge",
    thaiMassage: "Thai massage combines stretching and pressure techniques to improve flexibility and promote relaxation.",

    cup: "Cupping Therapy",
    cupMassage: "Cupping therapy uses suction cups to boost blood flow and relax muscles, leaving temporary marks.",

    antiCell: "Anti-cellulite massage",
    antiCellbio: "To reduce cellulite, improve circulation and break down fat.",

    spatr: "Spa treatments",
    spaTreat: "Relaxation and rejuvenation through massages, facials, and body treatments.",

    prenatal: "Prenatal massage",
    prenatalMassage: "Designed to relieve pregnancy discomfort and promote relaxation.",

    candle: "Candle experience",
    candleExperience: "Luxurious candle experience massage for deep relaxation and skin hydration.",

    signature: "Signature aromatherapy massage",
    signatureAromath: "Signature aromatherapy massage for ultimate relaxation and rejuvenation.",

    reflexology: "Reflexology massage",
    reflexologyMassage: "Reflexology sessions enhance wellness through foot and hand pressure point stimulation.",

    holistic: "Holistic approach massage",
    holisticApproach: "Holistic approach massage integrates body, mind, and spirit for comprehensive wellness.",

    sports: "Sports massage",
    sportsMassage: "Sports massage enhances athletic performance and aids in muscle recovery.",

    rejuvans: "Rejuvans massage",
    rejuvansMassage: "Rejuvans rejuvenation and relaxation with specialized techniques.",

    guaSha: "Gua sha massage",
    guaShaMassage: "Gua Sha enhances circulation and promotes healing through skin scraping techniques.",

    faceSculpture: "Face sculpture treatments",
    faceSculptureM: "Face sculpture treatments enhance facial features through advanced cosmetic techniques.",

    buccal: "Buccal face massage",
    buccalFace: "Buccal face massage focuses on cheek and jaw muscle relaxation and rejuvenation.",


    // Modal Big info

    bBig: "A back and shoulder massage is a therapeutic treatment that specifically targets the muscles and tissues of the back and shoulders, aiming to release tension, alleviate stiffness, and improve circulation. This deeply relaxing therapy not only promotes physical relaxation but also enhances mental well-being, providing a rejuvenating experience that helps restore balance and comfort to the body.",

    hbBig: "A head and back massage focuses on relieving tension and promoting relaxation in these areas. It combines gentle strokes and pressure techniques to alleviate stress and improve overall well-being. This therapeutic treatment not only targets physical discomfort but also enhances mental clarity, leaving you feeling rejuvenated and balanced.",

    fBig: "Facial massage acts like a natural lifting, as its techniques are designed to lift and revitalize every part of it. The massage is applied by a specialist to the face with the fingertips and aims to balance facial expressions and improve the quality of the skin.  muscle tension is released and good muscle tone is restored. Thus, the face regains its natural shape, while microcirculation improves, thus helping the transport of water, oxygen and nutrients to the cells. At the same time, toxins that have accumulated in the subcutaneous tissues and skin are removed",

    lBig: "Lymphatic drainage massage stimulates the lymphatic system to improve circulation and reduce swelling. Using gentle, rhythmic strokes, this massage helps detoxify the body, enhance immune function, and promote overall health. It's beneficial for reducing fluid retention and improving skin health, leaving you feeling refreshed and revitalized.",

    dBig: "Deep tissue massage targets deep muscle layers to relieve chronic tension and pain. Using slow, firm strokes, it helps break up scar tissue and improve blood flow. Ideal for muscle injuries or chronic pain, it promotes better mobility and muscle health. The focused pressure and deep techniques release muscle knots, leaving you feeling flexible and relaxed.",
    
    mBig: " Therapeutic massage is for the elimination of pain in the muscle skeletal system with the help of this treatment not only  Does the body become healthy, but also restores muscle weakness and skin atrophy. Moreover, it takes part in the restoration of the hormonal balance, and it improves the blood circulation in the brain. The purpose of this treatment is to feel energized and renewed therapeutic massage is used to combat illnesses, such as pain in the neck and back area, headaches and osteochondrosis.",
   
    jBig: "The massage is done with hot packs containing herbs. The technique followed is muscle relaxing, as the heat of the bags placed on the body, offer muscle relaxation and rejuvenation, removing the stress, concepts and anxiety of everyday life.",
   
    sBig: "Swedish massage is a popular therapeutic technique involving long strokes, kneading, and circular movements to relax muscles, improve circulation, and promote overall relaxation. It's known for its gentle yet effective approach in reducing tension and enhancing well-being.",
   
    hBig: "Hot stone massage involves placing heated stones on specific points of the body to relax muscles and promote relaxation. Therapists may also use the stones to massage the body, combining heat with pressure to alleviate tension and improve circulation. It's known for its soothing effects and ability to enhance overall well-being.",

    headBackBig: "Focusing on tension primarily in the back,neck, shoulders and head, this massage is designed to relieve deep-seated stress by combining specially blended massage oils chosen to suit your individual needs an exotic mix of warmth, texture and aroma tempts the body into blissful relaxation.",
    
    tBig: "A great way to activate your mind and body, Thai massage works like yoga. Improves blood circulation, A natural treatment for muscle and body aches. Improves sleep pattern and it reduces stress and anxiety. ",
   
    cBig: "Cupping therapy involves placing cups on the skin to create suction. This suction helps increase blood flow to the area, promoting healing and relieving muscle tension. It's often used to treat pain, inflammation, and improve circulation. The cups can be left in place or moved around depending on the desired therapeutic effect.",

    antiBig: "Anti-cellulite massage targets deep tissue to reduce the appearance of cellulite by improving blood flow and lymphatic drainage. Using firm pressure and specialized techniques, it aims to smooth skin texture and tighten areas affected by cellulite, promoting a more toned appearance over time.",

    spaBig: "Spa treatments encompass massages, facials, body wraps, and hydrotherapy, designed for relaxation and health benefits. They address stress, skin care, and muscle relaxation, promoting overall well-being through therapeutic techniques and soothing experiences.",

    prentBig: "Prenatal massage relieves discomfort and promotes relaxation for expectant mothers. It targets areas like the back, hips, and legs, easing muscle tension and improving circulation. Consult your healthcare provider before scheduling.",

    candlBig: "Candle experiences involve using candles for a soothing ambiance during activities like massages or relaxation. Their soft light and fragrance enhance calmness, commonly found in spas or personal self-care routines.",

    mmBig: "Signature aromatherapy massage blends essential oils with therapeutic techniques to relax the body and mind, relieve tension, and enhance well-being through aromas and touch.",

    footBig: "Reflexology massage targets pressure points on the feet, hands, and ears to promote relaxation, relieve tension, and improve overall well-being through stimulation of the body's natural healing processes.",

    holistBig: "Holistic approach massage integrates body, mind, and spirit to promote overall well-being. It combines various techniques to relax muscles, relieve stress, and restore balance, aiming for a comprehensive healing experience.",

    sportBig: "Sports massage focuses on treating muscles used for athletic performance. It aims to improve flexibility, prevent injuries, and enhance recovery through techniques like deep tissue massage and stretching.",

    rejuBig: "Rejuvans massage is designed to revitalize and rejuvenate the body, promoting relaxation and restoring energy through specialized techniques and soothing movements.",

    guaBig: "Gua sha massage involves scraping the skin with a tool to improve circulation and relieve muscle tension. It's used to reduce inflammation, promote healing, and enhance overall well-being.",

    faceBig: "Advantages of the Sculptural Face Lifting technique, It perfectly treats the sagging of the face, It helps to straighten the muscles in a natural way, It restores muscle tone and elasticity to normal, It tightens and smooths the skin restoring the oval of the face Improves blood circulation and lymph flow Activates metabolism and tissue nutrition, Eliminates spasms and burns",

    buccalBig: "There are many innovative beauty and wellness solutions available today and one of these is buccal massage. Tissues don’t receive enough nutrients and oxygen, resulting in water retention, puffiness, a dull complexion, and drooping cheeks and jowls. Buccal massage resolves these issues, providing numerous skin benefits.",

    // about 
    masseuse: "Masseuse",
    khBakuradze: "Khatia Bakuradze",
    aboutKhatia: "Certified specialized massage therapist;",
    qualifications: "Massage coach.",
    fPlace: "15 years of work experience.",
    sPlace: "Specializing in various techniques such as:",
    certified: "Certified Masseuse",
    bom: "Facial Massage",
    gusha: "Gua Sha",
    tmas: "Thai massage",
    cumasag: "Cupping Therapy",
    thermas: "Therapeiutic Massage",
    mhc: "Many happy customers;",
    wowmc: "Winner of World Massage Championship;",
    cN: "Contact Now",

    // Second carousel
    makeaGift: "Make a Gift",
    egw: "Easy gift-giving with beautifully packaged options.",
    tyl: "Treat your loved ones to relaxation.",
    tof: "Tailored options for every preference.",
    sfb: "Suitable for birthdays, anniversaries, holidays.",

    bbct: "Bachelorette",
    ifb: "Ideal for bonding and relaxation.",
    thb: "Treat the bride-to-be to luxury.",
    psf: "Pre-wedding skincare for the entire party.",
    grf: "Get ready for the big day in style.",

    // Testimonial Start

    testimonial: "Testimonial",
    hs: "Hotels Served",
    hotelName: "Hotel Name",

    testimonial1: "InterContinental",
    info1: "Khatia Bakuradze has been an incredible addition to our spa team. Her expertise in various massage techniques, especially her award-winning Lomi-Lomi massage, has received rave reviews from our guests. Her professionalism and dedication to providing exceptional service make her a true asset to our hotel.",

    testimonial2: "Masazaki in Glifada",
    info2: "Since joining us, Khatia has brought a new level of excellence to our spa services. Her skill in traditional Thai massage and therapeutic treatments has made a significant impact on our clients' well-being. We highly recommend Khatia for her outstanding commitment to holistic health and relaxation.",

    testimonial3: "Royal Mosxoy Spa",
    info3: "We are thrilled to have Khatia Bakuradze as part of our team. Her achievements, including her top honors at the World Massage Championship, speak volumes about her talent and dedication. Our clients consistently praise her deep tissue massages and facial rejuvenation treatments. Khatia truly elevates our spa experience.",

    // SS
    selectService: "Select A Service",
    s1: "Service 1",
    s2: "Service 1",
    s3: "Service 1",
    mApointment: "Make Appointment",

    // Team Start
    champion: "Champion",
    xYear: "X Year - Certified 1st Place",

    // Btns
    bInfo: "info",
    bClose: "Close",

    // h1
    servic: "Service",
    hachivments: "Her Achievements",

    // h6
    h6: "Spa Specialist",

    // a
    on: "Order Now",
    oNow: "Order Now",

    // Placeholders
    usrplaceholder: "Your Name",
    emailplaceholder: "Your Email",
    dataplaceholder: "Select Date",
    timeplaceholder: "Select Time",

  },
  el: {
    // Nav bar
    hometext: "Σπίτι",
    about: "σχετικά με",
    service: "Υπηρεσίες",
    masseuse: "μασέρ",
    contact: "Επικοινωνία",
    spa: "SPA & ΚΕΝΤΡΟ ΟΜΟΡΦΙΑΣ",
    h3treatment: "Θεραπεία Μασάζ",
    treatment: "Οι θεραπείες μασάζ χαλαρώνουν τους μύες, ανακουφίζουν την ένταση και βελτιώνουν την κυκλοφορία του αίματος, προάγοντας τη συνολική ευεξία.",
    Appointment: "Κλείστε Ραντεβού",
    gallery: "Εκθεσιακός χώρος",

    // ABOUT US
    aboutUs: "Σχετικά με εμάς",
    bestMm: "Ο καλύτερος μασέρ",
    eur: "Ζήστε απαράμιλλη χαλάρωση και ευεξία με τους εξαιρετικούς μασέρ μας: Ανεβάστε το ταξίδι σας σε νέα ύψη άνεσης, ανανέωσης και απόλυτης χαλάρωσης",
    hsrv: "Ξενοδοχεία που εξυπηρετούνται",
    hClients: "Ευτυχισμένοι πελάτες",

    // Service starts
    ourService: "Η ΥΠΗΡΕΣΙΑ ΜΑΣ",
    spaBeauty: "Υπηρεσίες Σπα και Ομορφιάς",
    
    backShoulder: "Μασάζ πλάτης και ώμων",
    backShoulderMassage: "Τα μασάζ στην πλάτη και τους ώμους είναι εξαιρετικά αποτελεσματικά για χαλάρωση των μυών και ανακούφιση της έντασης.",
    
    headBack: "Μασάζ κεφαλής και πλάτης",
    headBackMassage: "Δώστε προτεραιότητα στη ζωτικότητά σας και απολαύστε ένα αναζωογονητικό μασάζ κεφαλής και πλάτης σήμερα!",

    faceLifting: "Ανόρθωση προσώπου",
    faceLiftingMassage: "Αναζωογονηθείτε και ανανεωθείτε με ένα μασάζ κεφαλής και πλάτης και έναν θεραπευτικό χειρισμό ανόρθωσης προσώπου σήμερα!",

    lympatatic: "Μασάζ λεμφικής αποσυμφόρησης",
    lympataticMassage: "Το μασάζ λεμφικής αποσυμφόρησης βελτιώνει την κυκλοφορία, μειώνει το πρήξιμο και βοηθά στην αποτοξίνωση.",

    deepTissue: "Βαθύς ιστός",
    deepTissueMassage: "Το μασάζ βαθιού ιστού στοχεύει στους βαθιούς μύες για ανακούφιση της έντασης, ανακούφιση του πόνου και ενίσχυση της ευελιξίας.",

    medical: "Ιατρικές θεραπείες",
    medicalMassage: "Οι ιατρικές θεραπείες περιλαμβάνουν φάρμακα, χειρουργικές επεμβάσεις και προληπτική φροντίδα για τη διαχείριση και θεραπεία ασθενειών.",

    japanese: "ΜΑΣΑΖ με ΒΟΤΑΝΑ ΜΠΑΛΑ",
    japaneseMassage: "Το μασάζ με βότανα με μπάλα χρησιμοποιεί θερμαινόμενες κομπρέσες με βότανα για την ανακούφιση του μυϊκού πόνου και την ενίσχυση της χαλάρωσης.",

    swedish: "Σουηδικό μασάζ",
    swedishMassage: "Το Σουηδικό μασάζ χαλαρώνει τους μύες με απαλή πίεση και μακριές κινήσεις, προωθώντας την ευημερία.",

    hotStone: "Μασάζ με ζεστά πετράδια",
    hotStoneMassage: "Το μασάζ με ζεστά πετράδια χρησιμοποιεί θερμαντικά πετράδια για να χαλαρώσει τους μύες και να προάγει τη χαλάρωση και την ευημερία.",

    thai: "Μασάζ Ταϊλάνδης",
    thaiMassage: "Το Ταϊλανδέζικο μασάζ συνδυάζει τεχνικές εκτέλεσης και ασκήσεων πίεσης για τη βελτίωση της ευελιξίας και την προώθηση της χαλάρωσης.",

    cup: "Θεραπεία με κύπελλα",
    cupMassage: "Η θεραπεία με κύπελλα χρησιμοποιεί ειδικά κύπελλα για να αυξήσει τη ροή του αίματος και να χαλαρώσει τους μύες, αφήνοντας προσωρινά σημάδια.",

    antiCell: "Μασάζ κατά της κυτταρίτιδας",
    antiCellbio: "μειώνει την κυτταρίτιδα βελτιώνοντας την κυκλοφορία",

    spatr: "Περιποιήσεις σπα",
    spaTreat: "Χαλάρωση και αναζωογόνηση μέσω μασάζ, περιποιήσεων προσώπου και σώματος.",

    prenatal: "Προγεννητικό μασάζ",
    prenatalMassage: "Σχεδιασμένο για να ανακουφίζει την εγκυμοσύνη",

    candle: "Κερί εμπειρία",
    candleExperience: "Πολυτελές μασάζ εμπειρίας με κεριά για βαθιά χαλάρωση και ενυδάτωση του δέρματος.",

    signature: "Signature μασάζ αρωματοθεραπείας",
    signatureAromath: "Signature μασάζ αρωματοθεραπείας για απόλυτη χαλάρωση και αναζωογόνηση.",

    reflexology: "Ρεφλεξολογικό μασάζ",
    reflexologyMassage: "Οι συνεδρίες ρεφλεξολογίας ενισχύουν την ευεξία μέσω των ποδιών και των χεριών",

    holistic: "μασάζ ολιστικής προσέγγισης",
    holisticApproach: "Το μασάζ ολιστικής προσέγγισης ενσωματώνει σώμα, μυαλό και πνεύμα για ολοκληρωμένη ευεξία.",

    sports: "Αθλητικό μασάζ",
    sportsMassage: "Το αθλητικό μασάζ ενισχύει την αθλητική απόδοση και βοηθά στην αποκατάσταση των μυών.",

    rejuvans: "Μασάζ Rejuvans",
    rejuvansMassage: "Rejuvans αναζωογόνηση και χαλάρωση με εξειδικευμένες τεχνικές.",

    guaSha: "Μασάζ Gua sha",
    guaShaMassage: "Το Gua Sha ενισχύει την κυκλοφορία και προάγει την επούλωση μέσω τεχνικών απόξεσης δέρματος.",

    faceSculpture: "Θεραπείες γλυπτικής προσώπου",
    faceSculptureM: "Οι θεραπείες γλυπτικής προσώπου ενισχύουν τα χαρακτηριστικά του προσώπου μέσω προηγμένων καλλυντικών τεχνικών.",

    buccal: "Στοματικό μασάζ προσώπου",
    buccalFace: "Το στοματικό μασάζ προσώπου εστιάζει στη χαλάρωση και την αναζωογόνηση των μυών των μάγουλων και των γνάθων.",

    // Modal Big info

    bBig: "Το μασάζ πλάτης και ώμων είναι μια θεραπευτική θεραπεία που στοχεύει ειδικά τους μύες και τους ιστούς της πλάτης και των ώμων, με στόχο την απελευθέρωση της έντασης, την ανακούφιση της δυσκαμψίας και τη βελτίωση της κυκλοφορίας. Αυτή η βαθιά χαλαρωτική θεραπεία όχι μόνο προάγει τη σωματική χαλάρωση αλλά και ενισχύει την ψυχική ευεξία, παρέχοντας μια αναζωογονητική εμπειρία που βοηθά στην αποκατάσταση της ισορροπίας και της άνεσης στο σώμα.",

    hbBig: "Ένα μασάζ κεφαλής και πλάτης επικεντρώνεται στην ανακούφιση της έντασης και στην προώθηση της χαλάρωσης σε αυτές τις περιοχές. Συνδυάζει απαλά κτυπήματα και τεχνικές πίεσης για να ανακουφίσει το άγχος και να βελτιώσει τη συνολική ευεξία. Αυτή η θεραπευτική θεραπεία όχι μόνο στοχεύει στη σωματική δυσφορία, αλλά ενισχύει και την πνευματική διαύγεια, αφήνοντάς σας να αισθάνεστε αναζωογονημένοι και ισορροπημένοι.",

    fBig: "Το μασάζ προσώπου λειτουργεί σαν φυσικό lifting, ανασηκώνοντας και αναζωογονώντας το πρόσωπο. Εφαρμόζεται από ειδικό με τις άκρες των δαχτύλων, στοχεύοντας στην εξισορρόπηση των εκφράσεων και τη βελτίωση της επιδερμίδας. Η μυϊκή ένταση απελευθερώνεται, αποκαθιστώντας τον καλό μυϊκό τόνο, ενώ βελτιώνεται η μικροκυκλοφορία, μεταφέροντας νερό, οξυγόνο και θρεπτικά συστατικά στα κύτταρα και απομακρύνοντας τις τοξίνες από την επιδερμίδα.",

    lBig: "Το μασάζ λεμφικής παροχέτευσης διεγείρει το λεμφικό σύστημα για να βελτιώσει την κυκλοφορία και να μειώσει το πρήξιμο. Χρησιμοποιώντας απαλές, ρυθμικές κινήσεις, αυτό το μασάζ βοηθά στην αποτοξίνωση του σώματος, ενισχύει τη λειτουργία του ανοσοποιητικού και προάγει τη συνολική υγεία. Είναι ευεργετικό για τη μείωση της κατακράτησης υγρών και τη βελτίωση της υγείας του δέρματος, αφήνοντάς σας να αισθάνεστε αναζωογονημένοι και αναζωογονημένοι.",

    dBig: "Το βαθύ μασάζ ιστών στοχεύει στα βαθιά μυϊκά στρώματα για να ανακουφίσει τη χρόνια ένταση και τον πόνο. Χρησιμοποιώντας αργές, σταθερές κινήσεις, βοηθά στη διάσπαση του ουλώδους ιστού και στη βελτίωση της ροής του αίματος. Ιδανικό για μυϊκούς τραυματισμούς ή χρόνιους πόνους, προάγει την καλύτερη κινητικότητα και την υγεία των μυών. Η εστιασμένη πίεση και οι τεχνικές βαθιάς απελευθερώνουν τους μυϊκούς κόμπους, αφήνοντάς σας να αισθάνεστε ευέλικτοι και χαλαροί.",
    
    mBig: "Το θεραπευτικό μασάζ είναι για την εξάλειψη του πόνου στο μυϊκό σκελετικό σύστημα με τη βοήθεια αυτής της θεραπείας όχι μόνο Γίνεται υγιές το σώμα, αλλά και αποκαθιστά τη μυϊκή αδυναμία και την ατροφία του δέρματος. Επιπλέον, συμμετέχει στην αποκατάσταση της ορμονικής ισορροπίας και βελτιώνει την κυκλοφορία του αίματος στον εγκέφαλο. Ο σκοπός αυτής της θεραπείας είναι να αισθάνεστε γεμάτοι ενέργεια και το ανανεωμένο θεραπευτικό μασάζ χρησιμοποιείται για την καταπολέμηση ασθενειών, όπως πόνος στον αυχένα και την πλάτη, πονοκεφάλους και οστεοχονδρωσία.",
   
    jBig: "Το μασάζ γίνεται με ζεστά πουγκιά που περιέχουν βότανα. Η τεχνική που ακολουθείται είναι μυοχαλαρωτική, καθώς η ζέστη των πουγκιών που τοποθετούνται στο σώμα, προσφέρουν μυϊκή χαλάρωση και αναζωογόνηση, απομακρύνοντας το στρες, τις έννοιες και το άγχος της καθημερινότητας.",
   
    sBig: "Το σουηδικό μασάζ είναι μια δημοφιλής θεραπευτική τεχνική που περιλαμβάνει μεγάλα εγκεφαλικά επεισόδια, ζύμωμα και κυκλικές κινήσεις για χαλάρωση των μυών, βελτίωση της κυκλοφορίας και προώθηση της συνολικής χαλάρωσης. Είναι γνωστό για την ήπια αλλά αποτελεσματική προσέγγισή του στη μείωση της έντασης και την ενίσχυση της ευεξίας.",
   
    hBig: "Το μασάζ με ζεστές πέτρες περιλαμβάνει την τοποθέτηση θερμαινόμενων λίθων σε συγκεκριμένα σημεία του σώματος για χαλάρωση των μυών και προώθηση της χαλάρωσης. Οι θεραπευτές μπορούν επίσης να χρησιμοποιήσουν τις πέτρες για να κάνουν μασάζ στο σώμα, συνδυάζοντας τη θερμότητα με την πίεση για την ανακούφιση της έντασης και τη βελτίωση της κυκλοφορίας. Είναι γνωστό για τα καταπραϋντικά του αποτελέσματα και την ικανότητά του να βελτιώνει τη συνολική ευεξία.",

    headBackBig: "Εστιάζοντας στην ένταση κυρίως στην πλάτη, το λαιμό, τους ώμους και το κεφάλι, αυτό το μασάζ έχει σχεδιαστεί για να ανακουφίζει από το βαθύ στρες συνδυάζοντας ειδικά αναμεμειγμένα λάδια μασάζ που επιλέγονται για να ταιριάζουν στις ατομικές σας ανάγκες, ένα εξωτικό μείγμα ζεστασιάς, υφής και αρώματος που δελεάζει το σώμα σε ευτυχία. χαλάρωση.",
    
    tBig: "Ένας υπέροχος τρόπος για να ενεργοποιήσετε το μυαλό και το σώμα σας Το ταϊλανδέζικο μασάζ λειτουργεί όπως η γιόγκα. Βελτιώνει την κυκλοφορία του αίματος, Μια φυσική θεραπεία για πόνους μυών και σώματος, Βελτιώνει το μοτίβο ύπνου, Μειώνει το στρες και το άγχος.",
   
    cBig: "Η θεραπεία βεντούζας περιλαμβάνει την τοποθέτηση κυπέλλων στο δέρμα για τη δημιουργία αναρρόφησης. Αυτή η αναρρόφηση βοηθά στην αύξηση της ροής του αίματος στην περιοχή, προάγοντας την επούλωση και ανακουφίζοντας την ένταση των μυών. Συχνά χρησιμοποιείται για τη θεραπεία του πόνου, της φλεγμονής και τη βελτίωση της κυκλοφορίας. Τα κύπελλα μπορούν να μείνουν στη θέση τους ή να μετακινηθούν ανάλογα με το επιθυμητό θεραπευτικό αποτέλεσμα.",

    antiBig: "Το μασάζ κατά της κυτταρίτιδας στοχεύει στον βαθύ ιστό για να μειώσει την εμφάνιση της κυτταρίτιδας βελτιώνοντας τη ροή του αίματος και τη λεμφική παροχέτευση. Χρησιμοποιώντας συνεχή πίεση και εξειδικευμένες τεχνικές, στοχεύει στη λεία υφή του δέρματος και στη σύσφιξη των περιοχών που επηρεάζονται από την κυτταρίτιδα, προάγοντας μια πιο τονωμένη εμφάνιση με την πάροδο του χρόνου.",

    spaBig: "Οι θεραπείες σπα περιλαμβάνουν μασάζ, περιποιήσεις προσώπου, περιτυλίξεις σώματος και υδροθεραπεία, σχεδιασμένες για χαλάρωση και οφέλη για την υγεία. Αντιμετωπίζουν το άγχος, τη φροντίδα του δέρματος και τη μυϊκή χαλάρωση, προάγοντας τη συνολική ευεξία μέσω θεραπευτικών τεχνικών και καταπραϋντικών εμπειριών.",

    prentBig: "Το προγεννητικό μασάζ ανακουφίζει από την ταλαιπωρία και προάγει τη χαλάρωση για τις μέλλουσες μητέρες. Στοχεύει σε περιοχές όπως η πλάτη, οι γοφοί και τα πόδια, μειώνοντας την ένταση των μυών και βελτιώνοντας την κυκλοφορία. Συμβουλευτείτε τον πάροχο υγειονομικής περίθαλψης πριν προγραμματίσετε.",

    candlBig: "Οι εμπειρίες με κεριά περιλαμβάνουν τη χρήση κεριών για μια χαλαρωτική ατμόσφαιρα κατά τη διάρκεια δραστηριοτήτων όπως μασάζ ή χαλάρωση. Το απαλό φως και το άρωμά τους ενισχύουν την ηρεμία που συναντάμε συνήθως στα σπα ή στις προσωπικές ρουτίνες αυτοεξυπηρέτησης.",

    mmBig: "Το Signature Aromatherapy Massage συνδυάζει αιθέρια έλαια με θεραπευτικές τεχνικές για να χαλαρώσει το σώμα και το μυαλό, να ανακουφίσει την ένταση και να ενισχύσει την ευεξία μέσω των αρωμάτων και της αφής.",

    footBig: "Το ρεφλεξολογικό μασάζ στοχεύει τα σημεία πίεσης στα πόδια, τα χέρια και τα αυτιά για να προωθήσει τη χαλάρωση, να ανακουφίσει την ένταση και να βελτιώσει τη συνολική ευεξία μέσω της διέγερσης των φυσικών διαδικασιών επούλωσης του σώματος.",

    holistBig: "Το μασάζ ολιστικής προσέγγισης ενσωματώνει το σώμα, το μυαλό και το πνεύμα για να προάγει τη συνολική ευεξία. Συνδυάζει διάφορες τεχνικές για τη χαλάρωση των μυών, την ανακούφιση από το στρες και την αποκατάσταση της ισορροπίας, με στόχο μια ολοκληρωμένη εμπειρία θεραπείας.",

    sportBig: "Το αθλητικό μασάζ επικεντρώνεται στη θεραπεία των μυών που χρησιμοποιούνται για αθλητικές επιδόσεις. Στοχεύει στη βελτίωση της ευελιξίας, στην πρόληψη τραυματισμών και στην ενίσχυση της αποκατάστασης μέσω τεχνικών όπως το βαθύ μασάζ ιστών και οι διατάσεις.",

    rejuBig: "Το μασάζ Rejuvans έχει σχεδιαστεί για να αναζωογονεί και να αναζωογονεί το σώμα, προάγοντας τη χαλάρωση και την αποκατάσταση της ενέργειας μέσω εξειδικευμένων τεχνικών και καταπραϋντικών κινήσεων.",

    guaBig: "Το μασάζ Gua sha περιλαμβάνει ξύσιμο του δέρματος με ένα εργαλείο για τη βελτίωση της κυκλοφορίας και την ανακούφιση της μυϊκής έντασης. Χρησιμοποιείται για τη μείωση της φλεγμονής, την προώθηση της επούλωσης και τη βελτίωση της συνολικής ευεξίας.",

    faceBig: "Πλεονεκτήματα της τεχνικής Sculptural Face Lifting, Αντιμετωπίζει εξαιρετικά τη χαλάρωση του προσώπου, Βοηθάει στην ανόρθωση των μυών με φυσικό τρόπο, Επαναφέρει στο φυσιολογικό τον μυϊκό τόνο και την ελαστικότητα, Συσφίγγει και λειαίνει το δέρμα αποκαθιστώντας το οβάλ του προσώπου, Βελτιώνει την κυκλοφορία του αίματος και τη λεμφική ροή, Ενεργοποιεί το μεταβολισμό και τη θρέψη των ιστών, Εξαφανίζει σπασμούς και μπλοκαρίσματα",

    buccalBig: "Υπάρχουν πολλές καινοτόμες λύσεις ομορφιάς και ευεξίας που είναι διαθέσιμες σήμερα και μία από αυτές είναι το στοματικό μασάζ. Οι ιστοί δεν λαμβάνουν αρκετά θρεπτικά συστατικά και οξυγόνο, με αποτέλεσμα την κατακράτηση νερού, το πρήξιμο, μια θαμπή επιδερμίδα και τα πεσμένα μάγουλα και τα πόδια. Το στοματικό μασάζ επιλύει αυτά τα προβλήματα, παρέχοντας πολυάριθμα οφέλη για το δέρμα.",

    // about
    masseuse: "Μασέρ",
    khBakuradze: "Khatia Bakuradze",
    aboutKhatia: "Πιστοποιημένος εξειδικευμένος θεραπευτής μασάζ.",
    qualifications: "Προπονητής μασάζ.",
    fPlace: "15 χρόνια εργασιακή εμπειρία.",
    sPlace: "Εξειδίκευση σε διάφορες τεχνικές όπως:",
    certified: "Πιστοποιημένος μασέρ",
    bom: "Μασάζ με λάδι σώματος;",
    gusha: "Gua Sha;",
    tmas: "Ταϊλανδέζικο μασάζ;",
    cumasag: "βεντούζα;",
    thermas: "Θεραπευτικό Μασάζ",
    wwvh: "Συνεργάστηκε με διάφορα ξενοδοχεία.",
    mhc: "Πολλοί ευχαριστημένοι πελάτες.",
    wowmc: "Νικητής του Παγκόσμιου Πρωταθλήματος Μασάζ.",
    cN: "Επικοινώνησε τώρα",

    // second carousel
    makeaGift: "Κάντε ένα δώρο",
    egw: "Εύκολη δωροδοκία με όμορφες συσκευασίες επιλογών.",
    tyl: "Κάντε τους αγαπημένους σας να απολαύσουν χαλάρωση.",
    tof: "Εξατομικευμένες επιλογές για κάθε προτίμηση.",
    sfb: "Κατάλληλο για γενέθλια, επέτειους, διακοπές.",

    bbct: "Αγγελία",
    ifb: "Ιδανικό για δέσιμο και χαλάρωση.",
    thb: "Κάντε δώρο πολυτέλειας στη νύφη.",
    psf: "Προετοιμασία της επιδερμίδας πριν τον γάμο για ολόκληρο το πάρτι.",
    grf: "Προετοιμαστείτε για τη μεγάλη ημέρα με στυλ.",

    // Testimonial Start
    testimonial: "Μαρτυρία",
    hs: "Ξενοδοχεία που έχει εργαστεί",
    hotelName: "Όνομα Ξενοδοχείου",

    testimonial1: "InterContinental",
    info1: "Η Χατία Μπακουράτζε αποτελεί μια απίστευτη προσθήκη στην ομάδα μας στο σπα. Η ειδίκευσή της σε διάφορες τεχνικές μασάζ, ιδιαίτερα το βραβευμένο μασάζ Lomi-Lomi, έχει λάβει εξαιρετικές κριτικές από τους επισκέπτες μας. Η επαγγελματικότητά της και η αφοσίωσή της στην παροχή εξαιρετικής εξυπηρέτησης την καθιστούν πραγματικό πλεονέκτημα για το ξενοδοχείο μας.",

    testimonial2: "Masazaki in Glifada",
    info2: "Από τη στιγμή που έγινε μέρος της ομάδας μας, η Χατία έχει φέρει ένα νέο επίπεδο αριστείας στις υπηρεσίες του σπα μας. Η ειδίκευσή της στο παραδοσιακό Ταϊλανδέζικο μασάζ και στις θεραπευτικές θεραπείες έχει κάνει σημαντική επίδραση στην ευημερία των πελατών μας. Συνιστούμε θερμά την Χατία για την εξαιρετική της αφοσίωση στην ολιστική υγεία και χαλάρωση.",

    testimonial3: "Royal Mosxoy Spa",
    info3: "Είμαστε ενθουσιασμένοι που έχουμε την Χατία Μπακουράτζε στην ομάδα μας. Τα επιτεύγματά της, συμπεριλαμβανομένων των κορυφαίων της διακρίσεων στο Παγκόσμιο Πρωτάθλημα Μασάζ, μιλούν από μόνα τους για το ταλέντο και την αφοσίωσή της. Οι πελάτες μας συνεχώς επαινούν τα μασάζ βαθιάς ιστού και τις θεραπείες αναζωογόνησης προσώπου που προσφέρει. Η Χατία πραγματικά αναβαθμίζει την εμπειρία μας στο σπα.",

    // SS
    selectService: "Επιλέξτε έναν υπηρεσία",
    s1: "Υπηρεσία 1",
    s2: "Υπηρεσία 2",
    s3: "Υπηρεσία 3",
    mApointment: "Κλείστε ραντεβού",
    
    // Team Start
    champion: "Πρωταθλήτρια",
    xYear: "Έτος Χ - Πιστοποιημένη 1η Θέση",

    // Btns
    bInfo: "πληροφορίες",
    bClose: "κοντά",

    // h1
    servic: "Υπηρεσία",
    hachivments: "Τα επιτεύγματά της",

    // h6
    h6: "Ειδικός Σπα",

    // a
    on: "Παραγγείλτε τώρα",
    oNow: "Παράγγειλε τώρα",

    // Placeholders
    usrplaceholder: "Το όνομα σου",
    emailplaceholder: "Η διεύθυνση του ηλεκτρονικού σου ταχυδρομείου",
    dataplaceholder: "Επιλέξτε Ημερομηνία",
    timeplaceholder: "Επιλέξτε Ώρα",
  }
};

const switchLanguage = (lang) => {
  const elements = document.querySelectorAll('[data-translate]');
  elements.forEach(element => {
    const key = element.getAttribute('data-translate');
    if (translations[lang][key]) {
      if (element.tagName.toLowerCase() === 'input' || element.tagName.toLowerCase() === 'textarea') {
        element.placeholder = translations[lang][key];
      } else {
        element.innerText = translations[lang][key];
      }
    }
  });
};

const changeLanguage = (event) => {
  const selectedLanguage = event.target.getAttribute('data-lang');
  switchLanguage(selectedLanguage);
};

const flagsContainer = document.querySelector('.flags');
flagsContainer.addEventListener('click', changeLanguage);

window.onload = () => {
  switchLanguage('en');
};

